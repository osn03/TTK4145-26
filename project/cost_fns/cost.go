...
// Req represents a single hall request (up/down) at a given floor, plus which elevator it is assigned to.
type Req struct {
	Active     bool
	AssignedTo string
}
...
// State is an internal simulation state for one elevator: its ID, a snapshot of its elevator state, and a simulated time.
type State struct {
	ID    string
